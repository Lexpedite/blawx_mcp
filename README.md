...
- `blawx_legaldocs_list`, `blawx_legaldoc_create`, `blawx_legaldoc_detail`, `blawx_legaldoc_update`, `blawx_legaldoc_delete`
...
1. `blawx_encoding_guide` with topic `legaldocs`.
2. `blawx_legaldoc_create` or `blawx_legaldoc_update` with payload:
...
3. `blawx_legaldocpart_create` with payload such as:
...
1. Call `blawx_encoding_guide` (topic `quickstart`, then `encoding-process`, then `encodingpart`, then `blawx-json`).
	...
	- `encoding-process`: step-by-step workflow for creating and updating encoding parts
 	- `legaldocs`: LegalDoc and LegalDocPart structure, write fields, and project-selection workflow
